...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 主页管理 <span class="c-gray en">&gt;</span> 轮播图列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		...
		{{each data.list as good}}
			<tr>
				...
				<td style="text-align:center;vertical-align:middle;">
					{{if good.type=="VIDEO"}}<a href="${path}/page/recommend/video/list?id={{good.relationId}}">{{good.relationId}} 查看</a>
					{{else if good.type=="CURRICULUM"}}<a href="${path}/page/recommend/live/course/list?id={{good.relationId}}">{{good.relationId}} 查看</a>
					{{else if good.type =="ACTIVITY"}}<a href="${path}/page/recommend/activity/list?id={{good.relationId}}">{{good.relationId}} 查看</a>
					{{else if good.type =="LIVE"}}<a href="${path}/page/recommend/live/list?id={{good.relationId}}">{{good.relationId}} 查看</a>
					{{else}}{{/if}}
				</td>
				<td style="text-align:center;vertical-align:middle;">{{if good.type=="LIVE"}}<font color="#adff2f">直播</font>{{else if good.type=="VIDEO"}}<font color="blue">视频</font>{{else if good.type=="CURRICULUM"}}<font color="red">课程</font>
					{{else  if good.type=="ACTIVITY" }}<font color="yellow">活动</font>{{/if}}</td>
