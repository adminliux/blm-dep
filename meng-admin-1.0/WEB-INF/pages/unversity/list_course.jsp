...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>大学管理<span class="c-gray en">&gt;</span> 大学列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<form id="From">
		...
		<div class="mt-20" >
			...
			<script type="text/html" id="f1">
				...
				<tr>
					...
					<td style="text-align:center;vertical-align:middle;"><button type="button" class="btn btn-success" onclick="member_up(this,'{{good.id}})','{{good.sort}}">上移</button><button type="button" class="btn btn-success" onclick="member_down(this,'{{good.id}})','{{good.sort}}')">下移</button></td>
					...
					<td>{{if good.unLook==NULL}}无{{else if good.unLook!=NULL}}<a class="sit-preview" href="{{good.unLook}}" onMouseOver="biggerr()"><img src="{{good.unLook}}" data-preview-url="{{good.unLook}}" style="cursor:pointer;width:60px;height:60px;"/></a>{{/if}}</td>
					<td style="text-align:center;vertical-align:middle;">{{$timestampFormatYMDHM good.gmtDatetime}}</td>
					<td style="text-align:center;vertical-align:middle;">
						<u style="cursor:pointer" class="text-primary" onclick="layer_show('课程详情','${path}/page/unversity/list/course?id={{good.id}}','800','600')"><font color="#4169e1">查看</font></u>
					</td>

					<td style="text-align:center;vertical-align:middle;"><button type="button" class="btn btn-success" onclick="layer_show('修改大学','${path}/page/unversity/update/un?id={{good.id}}','800','600')">修改</button><button type="button" class="btn btn-success" onclick="drops({{good.id}})">删除</button></td>
				</tr>
				{{/each}}
			</script>
		...
		<script type="text/javascript">
            ...
            function drops(id) {
				...
				if(id==1){
                    layer.msg('不能删除百乐萌大学!',{icon: 5,time:1000});
                    return false;
				}
