...
<div class="page-container" id="classifyList2"></div>
<script type="text/html"  id="f5">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" placeholder="课程价格" name="price"  value="{{data.price}}">
		</div>
	</div>
	<br>
...
&nbsp课程时间：
<input type="text" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="beginTime" style="width: 10%; height: 30px" placeholder="开始时间" value="{{$timestampFormatYMDHM good.beginTime}}"/>
-
<input type="text" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" name="overTime" style="width: 10%; height: 30px" placeholder="结束时间" value="{{$timestampFormatYMDHM good.overTime}}"/>

<div class="row cl">
	<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>课程简介：</label>
	<div class="formControls col-xs-8 col-sm-9">
		<input type="text" class="input-text" placeholder="老师ID" name="title" value="{{good.roomDesc}}">
	</div>
</div>
<br>
...
<div class="row cl">
	...
	<div class="formControls col-xs-8 col-sm-9">
		<img width="120" height="120" id="zhaopian" src="previewImg">
...
<script type="text/javascript">
    ...
    function preview(file) {
        ...
        Project.ajaxUploadVideoFile('/video/upload/tenxun', fd, null, true).ajaxOK(function (data) {
            $("#zhaopian").attr("src",data.msg);
            $("input[name=previewImg]").val(data.msg);
        }, true);
    }
</script>
</body>
